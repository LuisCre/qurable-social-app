...
// ── Platform icons (monochrome SVG, uses currentColor) ────────────────────────
              ...
              <div style={{ display: 'flex', gap: 5 }}>
                {Object.entries(PLATFORMS).map(([k, v]) => {
                  ...
                  return (
                    <button key={k}
                      title={v.label}
                      onClick={() => { setPlatform(k); setFormatKey(v.formats[0]) }}
                      style={{
                        flex: 1, height: 34,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        borderRadius: 7, border: `1px solid ${isActive ? C.accent : C.inputBorder}`,
                        background: isActive ? 'transparent' : C.input,
